...
ll a[maxn],bef[maxn],aft[maxn],val[maxn],mxx[maxn],mnn[maxn];
void getsa(){
    int *x=t,*y=t1;
...
int main(){
    ...
    for(int k=n-1;k>=0;k--){
        for(int j=0;j<e[k].size();j++){
            ...
            val[bef[i]]+=val[i];
            cz[i]=true;
            if(mxx[bef[i]]!=0 && mxx[i]!=0)mx=max(mx,mxx[bef[i]]*mxx[i]);
            if(mnn[bef[i]]!=0 && mnn[i]!=0)mx=max(mx,mnn[bef[i]]*mnn[i]);
            mxx[bef[i]]=max(mxx[bef[i]],mxx[i]);
            mnn[bef[i]]=min(mnn[bef[i]],mnn[i]);
        }
