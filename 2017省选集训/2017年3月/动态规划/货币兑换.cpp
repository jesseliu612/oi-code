...
void solve(int l,int r){
    ...
    solve(l,mid);
    /*printf("use the followings : ");
    for(int i=l;i<=mid;i++){
        printf("%d(%.3lf,%.3lf,%.3lf) ",t[i].id,t[i].k,t[i].b,t[i].x);
    }
    printf("\nto update the followings : ");
    for(int i=mid+1;i<=r;i++){
        printf("%d(%.3lf,%.3lf) ",t[i].id,t[i].k,t[i].x);
    }
    cout << endl;*/
    int hd=0,tl=0;
    for(int i=l;i<=mid;++i){
        while(tl-hd>1 && (t[i].b-q[tl-1].b)*(q[tl-1].k-q[tl].k)>=(q[tl].b-q[tl-1].b)*(q[tl-1].k-t[i].k)){
            //printf("%.3lf,%.3lf\n",(t[i].b-q[tl-1].b)/(q[tl-1].k-t[i].k),(q[tl].b-q[tl-1].b)/(q[tl-1].k-q[tl].k));
            --tl;
        }
        q[++tl]=t[i];
    }
    /*printf("the followings are in the graph : ");
    for(int i=hd+1;i<=tl;i++){
        printf("%d(%.3lf,%.3lf,%.3lf) ",q[i].id,q[i].k,q[i].b,q[i].x);
    }
    cout << endl;*/
    for(int i=mid+1;i<=r;++i){
        ...
        /*for(int j=1;j<=tl;j++){
            printf("f[%d] = %.3lf , j = %d\n",cur,b[cur]*(q[j].k*x+q[j].b),q[j].id);
        }
        */
